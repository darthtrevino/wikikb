...
const LEXCAT_INDEXED_RE = /^\s*(?:indexed|synced)\s+(\d+)\s+chunk\(s\)/im;
...
# LexCAT prunes any term whose document frequency is below analyzer_min_vocab,
# counted over chunks. At the upstream default of 2, a term that appears in a
# single chunk is dropped from the vocabulary and becomes permanently
# unsearchable -- which is exactly the rare identifier or name a knowledge base
# is searched for. Retrieval then returns no hits and exits 0. Index every term.
...
async function runLexcatIndex(target: KbTarget, force = false, quiet = false): Promise<void> {
  ...
  try {
    ...
    const output = runLexcatCommand(
      runtime,
      incremental
        ? ["--index", pendingDb, "sync", staged.corpusDir]
...
function assertIndexIsQueryable(output: string, target: KbTarget): void {
  // `build` and `sync` both report the resulting chunk count on stdout. An
  // empty corpus is not an error to LexCAT -- it writes a valid index holding
  // nothing and every later query returns no hits and exits 0 -- so the count
  // is checked here rather than letting retrieval fail silently.
  const reported = output.match(LEXCAT_INDEXED_RE);
  if (!reported) {
    throw new Error(`LexCAT did not report an indexed chunk count for ${targetLabel(target)}.`);
  }
